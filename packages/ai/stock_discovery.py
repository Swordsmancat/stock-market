...
def build_stock_discovery_prompt(
    *,
    ...
    return (
        ...
        "below, written inline in square brackets. State material evidence gaps.\n\n"
        ...
        "Write concise markdown sections for Shortlist comparison, Evidence gaps, and Safety boundary."
    )
...
def build_deterministic_stock_discovery_explanation(
    *,
    ...
    for rank, item in enumerate(shortlist, start=1):
        ...
        lines.append(
            f"{rank}. `{symbol}` — score {score}; matched: {', '.join(rule_codes) or 'none'}. "
            f"{citation_suffix}".rstrip()
        )
