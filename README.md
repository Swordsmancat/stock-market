...
Evidence Center now loads `GET /official-disclosures/evidence-status` for the active A-share watchlist. `POST /official-disclosures/watchlist/ingest` queues a TaskRun with a 30-day/20-document default, rejects duplicate running batches, and processes CNINFO calls one at a time. `DISCLOSURE_BATCH_REQUEST_DELAY_MS` controls the inter-request delay; the worker enforces a 250 ms minimum. This is an explicit user action, not a Celery Beat schedule.
