...
| [docs/runbooks/a-share-research-coverage.md](docs/runbooks/a-share-research-coverage.md) | Full A-share universe sync, transparent discovery profiles, AI ranking boundary, corporate-action batches, and failure semantics |
...
- **Persisted market daily evidence**: provider-normalized stock fund flow, limit-up, Dragon Tiger List, block-trade, and hot-sector rows can be manually imported into stable `market_daily_event:*` citations; live/mock rows remain non-citable
- **Comprehensive A-share discovery**: failure-safe SSE/SZSE/BSE universe sync, uncapped pre-evaluation screening with bulk local evidence loading, coverage diagnostics, transparent named profiles, and bounded deterministic shortlists
- **Validated AI shortlist explanation**: AI can explain but cannot add, remove, or reorder deterministic candidates; unknown candidate symbols or citation IDs trigger a deterministic fallback
- **Corporate-action evidence**: report-period/cursor batches persist dividend/bonus-transfer and rights-allotment rows with partial-success retry diagnostics and stable `market_daily_event:*` citations
...
| P1 | Official disclosure evidence operations | Implemented / watchlist MVP | Evidence Center shows CNINFO metadata/document/section coverage for active A-share watchlist symbols, supports exact single-PDF ingestion, and queues a deduplicated TaskRun batch. The worker refreshes metadata and missing text PDFs strictly sequentially with bounded lookback, document count, and request delay; it never widens to the full universe. |
...
Evidence Center loads `GET /official-disclosures/evidence-status` for the active A-share watchlist. `POST /official-disclosures/watchlist/ingest` queues an explicit bounded TaskRun, while `POST /official-disclosures/watchlist/monitor` queues the incremental mode used by the operator control and Celery Beat. Both paths share one active-run mutex and process CNINFO calls sequentially with a 30-day/20-document default. Incremental refresh stores a per-symbol publication/document watermark, queries a three-day overlap so late or same-timestamp announcements remain discoverable, and relies on official identity plus document extraction state to avoid duplicate metadata or repeated extracted-PDF downloads.

`DISCLOSURE_MONITOR_ENABLED`, `DISCLOSURE_MONITOR_INTERVAL_MINUTES`, `DISCLOSURE_MONITOR_LOOKBACK_DAYS`, `DISCLOSURE_MONITOR_OVERLAP_DAYS`, `DISCLOSURE_MONITOR_MAX_DOCUMENTS`, and `DISCLOSURE_MONITOR_FRESHNESS_HOURS` control the schedule and SLA. Failed symbols keep their previous cursor and use bounded exponential retry configured by `DISCLOSURE_MONITOR_RETRY_BASE_MINUTES` and `DISCLOSURE_MONITOR_RETRY_MAX_MINUTES`. `DISCLOSURE_BATCH_REQUEST_DELAY_MS` controls the inter-request delay; the worker enforces a 250 ms minimum. Newly discovered announcements appear as research-review evidence only and never generate automatic investment conclusions or trading actions.
