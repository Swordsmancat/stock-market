  ...
  return levels.map((level, levelIndex) => (
    <TableRow key={`${level.price ?? "level"}-${levelIndex}`}>
      <TableCell className="font-medium">
        {formatMarketDepthLevelValue(level.price, locale, unavailableLabel)}
      </TableCell>
      <TableCell>
        {formatNumber(level.volume, locale, unavailableLabel, 0)}
      </TableCell>
      <TableCell>
        {formatNumber(level.amount, locale, unavailableLabel, 2)}
      </TableCell>
      <TableCell>
        {formatNumber(level.order_count, locale, unavailableLabel, 0)}
      </TableCell>
    </TableRow>
  ));
}
...
function renderRecentTradeRows({
  ...
  return trades.map((trade, tradeIndex) => (
    <TableRow key={`${trade.timestamp ?? "trade"}-${tradeIndex}`}>
      ...
      <TableCell className="font-medium">
        {formatMarketDepthLevelValue(trade.price, locale, unavailableLabel)}
      </TableCell>
      <TableCell>
        {formatNumber(trade.volume, locale, unavailableLabel, 0)}
      </TableCell>
      <TableCell>
        {formatNumber(trade.amount, locale, unavailableLabel, 2)}
      </TableCell>
    </TableRow>
  ));
}
...
function renderLargeOrderRows({
  ...
  return largeOrders.map((largeOrder, largeOrderIndex) => (
    ...
    >
      ...
      <TableCell>
        {formatNumber(largeOrder.volume, locale, unavailableLabel, 0)}
      </TableCell>
      <TableCell>
        {formatNumber(largeOrder.amount, locale, unavailableLabel, 2)}
      </TableCell>
    </TableRow>
  ));
}
...
export function MarketDepthCard({
  ...
  const providerLabel =
    marketDepth?.effective_provider ??
    marketDepth?.provider ??
    marketDepth?.requested_provider ??
    unavailableLabel;
  const availabilityReason =
    marketDepth?.availability?.reason ??
    marketDepth?.order_book?.reason ??
    t("marketDepthUnavailable");
      ...
      <FinancialTerminalCardContent className="space-y-6">
          ...
          <div>
            ...
            {marketDepth?.is_realtime
              ...
              : marketDepth?.is_delayed
                ? t("marketDepthDelayed", {
                    minutes: marketDepth.delay_minutes ?? 0,
                  })
                : unavailableLabel}
          </div>
        </div>

        <FinancialTerminalSurface className="bg-muted/20 p-3 text-sm text-muted-foreground">
          {availabilityReason}
        </FinancialTerminalSurface>
        ...
        {capabilities ? (
          <div
            ...
            aria-label={t("marketDepthCapabilities")}
          >
            {Object.entries(capabilities).map(
              ([capabilityName, isAvailable]) => (
                <Badge
                  key={capabilityName}
                  variant={isAvailable ? "default" : "outline"}
                >
                  {t(`marketDepthCapability${capabilityName}` as never)}:{" "}
                  {isAvailable
                    ? t("marketDepthCapabilityAvailable")
                    : t("marketDepthCapabilityUnavailable")}
                </Badge>
              ),
            )}
          </div>
        ...
        <section className="space-y-3">
          ...
          <SectionStatusMessage
            status={marketDepth?.order_book.status}
            reason={marketDepth?.order_book.reason ?? null}
          />
        ...
        <section className="space-y-3">
          <h3 className="text-base font-semibold">
            {t("marketDepthRecentTradesTitle")}
          </h3>
          <SectionStatusMessage
            status={marketDepth?.recent_trades.status}
            reason={marketDepth?.recent_trades.reason ?? null}
          />
          <Table>
            <TableHeader>
              <TableRow>
        ...
        <section className="space-y-3">
          ...
          <SectionStatusMessage
            status={marketDepth?.large_orders.status}
            reason={marketDepth?.large_orders.reason ?? null}
          />
          <Table>
            <TableHeader>
              <TableRow>
        ...
        <section className="space-y-3">
          <h3 className="text-base font-semibold">
            {t("marketDepthFundFlowTitle")}
          </h3>
          <SectionStatusMessage
            status={marketDepth?.fund_flow.status}
            reason={marketDepth?.fund_flow.reason ?? null}
          />
          <div className="grid gap-3 text-sm md:grid-cols-2 lg:grid-cols-4">
            <FinancialTerminalSurface className="p-3">
              <div className="text-muted-foreground">
                {t("marketDepthNetInflow")}
              </div>
            ...
            <FinancialTerminalSurface className="p-3">
              <div className="text-muted-foreground">
                {t("marketDepthRetailNetInflow")}
              </div>
              <div className="font-mono font-semibold">
                {formatNumber(
                  fundFlow?.retail_net_inflow,
                  locale,
                  unavailableLabel,
                  2,
                )}
            ...
            <FinancialTerminalSurface className="p-3">
              <div className="text-muted-foreground">
                {t("marketDepthSourceDefinition")}
              </div>
              <div className="font-semibold">
                {fundFlow?.source_definition ?? unavailableLabel}
              </div>
            </FinancialTerminalSurface>
          </div>
        </section>
