...
export async function GET(request: Request) {
  ...
  try {
    const response = await backendFetch(
      `/market-daily-data/limit-up-reasons?${backendSearchParams.toString()}`,
      { cache: "no-store" },
    );

    if (!response.ok) {
      return Response.json(
        unavailableMarketDailyDataPayload("Failed to fetch limit-up reason data"),
        { status: 502 },
      );
    }
    ...
    console.error("Limit-up reasons API error:", error);
