...
export async function GET(request: Request) {
  ...
  try {
    const response = await backendFetch(
      `/market-daily-data/block-trades?${backendSearchParams.toString()}`,
      { cache: "no-store" },
    );

    if (!response.ok) {
      return Response.json(
        unavailableMarketDailyDataPayload("Failed to fetch block-trade data"),
        { status: 502 },
      );
    }
    ...
    console.error("Block trades API error:", error);
