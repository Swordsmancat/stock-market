...
def test_hot_sectors_passes_provider_to_service(monkeypatch):
    ...
    def get_hot_sectors_payload_stub(limit: int, provider_name: str | None = None) -> dict[str, object]:
    ...
    response = client.get("/sectors/hot", params={"limit": 2, "provider": "fake_provider"})

    assert response.status_code == 200
    payload = response.json()
    assert captured_arguments == {"limit": 2, "provider_name": "fake_provider"}
