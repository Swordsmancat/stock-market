...
在 `/settings` 的“模型 API”区域选择一个预设即可启用现有 AI 能力：

- **DeepSeek**：自动使用 `https://api.deepseek.com/v1` 和 `deepseek-chat`。
- **OpenAI**：自动使用 `https://api.openai.com/v1` 和 `gpt-4o-mini`。
- **自定义 OpenAI-compatible**：在高级配置中填写 HTTP(S) API 地址和模型名。API 地址不能包含用户名、密码、查询参数或 URL 片段；鉴权信息只填写在 API Key 字段。
- **停用 AI API**：保留确定性 fallback，不影响本地选股、证据覆盖或历史记录。

旧配置中缺失或留空的 API 地址会继续使用默认值；若地址非空但格式非法，系统会停用外部模型调用，并要求重新输入密钥，避免把旧服务的密钥发送到其他默认主机。修正地址、重新选择预设并填写对应密钥后即可恢复。

API Key 只需在首次配置或更换时填写。保存后页面和公开设置接口只显示“已配置”，不会回显 Key；留空再次保存会保留原 Key。模型只负责解释和总结已有证据，不能新增、删除或重排选股候选，也不能绕过 citation 校验或 95/90/80 就绪门槛。
...
完整操作 runbook 位于 `docs/runbooks/official-macro-refresh.md`。建议先按 runbook 做 dry-run，再写入本地 observation，最后到首页和 `/evidence` 验证 value、as-of、source 和 metadata。
...
完整操作 runbook 位于 `docs/runbooks/official-macro-refresh.md`。World Bank 不需要 API key，但数据是年度发布、天然滞后；适合作为巴菲特指标的区域估值背景，不应理解为实时行情。
...
- 当前 MVP 主要基于日线、平台内已验证上下文、人工复核来源笔记、CNINFO 公告元数据和已提取的官方文本型 PDF 页级章节；扫描件 OCR、transcripts、向量检索、全市场文档回填和 watchlist 级监控仍是后续能力。
...
| AlphaSense 类研究产品 | 对 filings、transcripts、新闻和研究资料做 AI 搜索、监控、摘要和引用 | 已有 citation-aware dashboard narrative、已保存研究摘要收件箱、AI 报告、个股 AI assistant 的引用校验、人工复核来源笔记本、CNINFO 元数据引用和文本型 PDF 页级章节引用 | 还缺 OCR、批量文档监控、向量检索、transcripts、主题追踪和更细的摘要筛选。 |
