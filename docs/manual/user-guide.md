...
| Phase 3 | AI 助手 | MVP 可用 | 个股详情页已提供聊天式 AI 市场助手，可基于可验证日线、指标、基本面和新闻上下文回答问题，并显示引用、诊断和免责声明。 |
...
- 后端通过 `POST /assistant/market` 聚合日线、指标、基本面和新闻舆情上下文。
...
- 当前 MVP 主要基于日线和平台内已验证上下文；实时行情、分时、Level-2、逐笔和资金流仍取决于后续 provider 管线。
...
| AI 研究 | 有报告/摘要和 AI 市场助手 MVP | 可追踪引用、文档检索、会议纪要、实时问答 | 助手已具备上下文问答入口，但仍缺少文档检索、实时行情联动和更丰富的引用来源。 |
| 运维可用性 | 有任务监控、provider readiness、degraded 状态 | 数据 SLA、权限管理、审计日志、告警 | 缺少生产级数据 SLA 和权限模型。 |

## 优先优化路线

1. **AI 市场助手增强**：扩展多轮上下文、报告/新闻检索、更多引用来源和实时行情联动。
