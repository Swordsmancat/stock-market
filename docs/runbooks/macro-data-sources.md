...
| `cn_fr007` | FR007 银行间质押式回购定盘利率 | `repo_rate_hist` | 中国货币网回购定盘利率 | `date` | `FR007` |
| `cn_fdr007` | FDR007 存款类机构质押式回购定盘利率 | `repo_rate_hist` | 中国货币网回购定盘利率 | `date` | `FDR007` |
...
| 回购定盘利率 | <https://www.chinamoney.com.cn/chinese/bkfrr/> |
...
通用 DR007、国债期货、指数 PE/股息率、详细失业率、商品价格和财政支出尚未进入生产映射。当前已接入的是上游明确命名的 `FR007` 与 `FDR007`，不得把 `FDR007` 静默改名为 `DR007`。其余接入前必须先验证来源语义和运行时字段，再按上述流程新增，不能仅根据页面截图填充。

## 国家统计局直连候选源资格审查（2026-07-17）

另一个系统提供了国家统计局
`POST /dg/website/publicrelease/web/external/stream/esData` 的接入思路，适合在完整映射可验证后作为中国宏观数据的官方降级源；目前**尚未进入生产**：

- 旧 `easyquery` 数据请求在当前运行环境返回 HTTP 403。
- 新 `esData` 端点无需登录即可到达，但请求必须携带有效的 `cid`、
  `rootId` 和完整 `indicatorIds`；空 JSON 返回 HTTP 500。
- 被审阅的外部文档省略了失业率的 9 个 `indicatorIds`，并缩写了贸易、
  货币和财政数据集的 `cid`。仅凭中文名称或卡片数值不能建立可靠映射。

因此不得把省略值、猜测值或浏览器 Cookie 写入 Provider。解除阻塞需要为每个
目标序列取得一份官方响应和完整请求标识，并逐项核对显示名、频率、单位、累计/
当期口径和官方页面数值。届时应新增独立 NBS Provider，通过显式 POST 刷新写入
`MarketIndicatorObservation`；普通 GET 继续只读数据库，失败保留最后一次成功值。
在这些证据齐备前，重叠指标继续使用当前 AkShare 适配器。
